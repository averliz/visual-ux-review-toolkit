...
1. Write a `findings.json` matching `references/report-schema.md`. Each finding's `box` must use the **document-relative** coordinates captured in Phase 1 (the geometry snippet already adds `scrollX/Y`). For a single page use the top-level form; to review several screens in one report, use the `pages[]` form (each page gets its own screenshots + findings, and the HTML gains a page switcher). `verdict`/`summary` auto-derive — don't hand-set them.
2. Run the generator:
   ```bash
